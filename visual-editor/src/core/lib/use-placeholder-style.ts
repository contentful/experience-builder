...
export const usePlaceholderStyle = () => {
  ...
  const onDragStartOrUpdate = (draggedItem: DragStart & Partial<DragUpdate>) => {
    ...
    const isSameDroppable = draggedItem.source.droppableId === draggedItem.destination?.droppableId;
