...
export const useResolvedData = (data: Data, config: Config, dispatch: Dispatch<PuckAction>) => {
  const [{ resolverKey, newData }, setResolverState] = useState({
    resolverKey: 0,
    newData: data,
  });

  const [componentState, setComponentState] = useState<Record<string, { loading: boolean }>>({});

  const deferredSetStates: Record<string, NodeJS.Timeout> = {};

  const setComponentLoading = useCallback((id: string, loading: boolean, defer: number = 0) => {
