...
export const convertPathDataToBreadcrumbs = (
  ...
  const currentPathData =
    pathData && id && pathData[id] ? { ...pathData[id] } : { label: 'Page', path: [] };
  ...
  return currentPathData?.path.reduce<Breadcrumb[]>((acc, zoneCompound) => {
    ...
    const itemIndex = parentZone.findIndex((queryItem) => queryItem.props.id === area);
...
export const useBreadcrumbs = (renderCount?: number) => {
  ...
  return useMemo<Breadcrumb[]>(() => {
    const breadcrumbs = convertPathDataToBreadcrumbs(selectedItem, dzContext?.pathData, data);
