...
// import { isAsset, isEntry } from '@contentful/experiences-sdk-react';
// import { isExperienceEntry } from '@contentful/experiences-core';
import type { UnresolvedLink as Link, Entry, Asset } from 'contentful';
...
export const isPatternEntry = (entry: Entry | ExperienceEntry): entry is ExperienceEntry => {
  return (
    ...
    !!entry.fields?.componentSettings && // signals that this is pattern (not experience) entry
...
const excludeAssets = (entity: Entry | Asset) => entity.sys.type !== 'Asset';
const excludePatternEntries = (entry: Entry) => !isPatternEntry(entry);

const isEntryLink = (link: Link<'Asset' | 'Entry'>): link is Link<'Entry'> =>
  link.sys.linkType === 'Entry';
const isAssetLink = (link: Link<'Asset' | 'Entry'>): link is Link<'Asset'> =>
