...
import type { ExperienceEntry, ExperienceFields, ExperienceComponentSettings } from '@/types';
...
// Currently sideloading is needed for prebinding default values
function addDefaultValueToDataSource(
  ppdID: string,
  ppd: PatternPropertyDefinition,
  dataSource: ExperienceFields['dataSource'],
): undefined | ExperienceFields['dataSource'] {
  if (!ppd.defaultValue) {
  ...
  const newDataSource: ExperienceFields['dataSource'] = {
    ...dataSource,
 ...
 */
export const sideloadPrebindingDefaultValues = (patternEntry: ExperienceEntry): void => {
  ...
  let dataSource: ExperienceFields['dataSource'] = patternEntry.fields.dataSource;

  // Sideload all default values for the parent pattern
  const patternDefs: PatternPropertyDefinitions =
    patternEntry.fields.componentSettings?.patternPropertyDefinitions ?? {};

  Object.entries(patternDefs).forEach(([ppdID, ppd]) => {
    dataSource = addDefaultValueToDataSource(ppdID, ppd, dataSource) || dataSource;
  });
  ...
  patternEntry.fields.dataSource = dataSource;
};
